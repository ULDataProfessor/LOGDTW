...
def demo_tw2002_navigation():
    ...
    for sector in test_jumps:
        ...
        if world.can_jump_to_sector(sector):
            ...
            if result['success']:
                ...
                console.print(f"[red]✗ Failed to jump to Sector {sector}: {result['message']}[/red]")
        else:
    ...
    if available_jumps:
        for jump in available_jumps:
            console.print(f"  • Sector {jump['sector']} ({jump['type'].upper()}) - {jump['faction']}")
            console.print(f"    Fuel: {jump['fuel_cost']}, Time: {jump['travel_time']}min, Danger: {jump['danger_level']}/10")
    else:
