...
class Player(db.Model):
    ...
    sector_visibility = relationship(
        "SectorVisibility", back_populates="player", cascade="all, delete-orphan"
    )
    event_history = relationship(
        "EventHistory", back_populates="player", cascade="all, delete-orphan"
    )
    ...
    def __init__(self, session_id: str, **kwargs):
        ...
        self.reputation_json = json.dumps(
            {"Federation": 0, "Empire": 0, "Pirates": 0, "Traders": 0}
        )

        self.skills_json = json.dumps({"Combat": 1, "Piloting": 1, "Trading": 1, "Engineering": 1})

        self.ship_data_json = json.dumps(
            {"hull_integrity": 100, "shield_power": 100, "weapon_slots": 2, "cargo_capacity": 50}
        )
...
def cleanup_old_sessions(hours: int = 24):
    ...
    old_players = Player.query.filter(Player.last_active < cutoff_time).all()
