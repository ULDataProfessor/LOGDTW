...
def test_travel_endpoint():
    response = client.post('/api/travel', json={'sector': 2})
    assert response.status_code == 200
    data = response.json()
    assert data['success']
...
def test_trade_buy_sell_cycle():
    buy = client.post('/api/trade', json={'item': 'Food', 'quantity': 1, 'trade_action': 'buy'})
    assert buy.status_code == 200
    assert buy.json()['success']
    sell = client.post('/api/trade', json={'item': 'Food', 'quantity': 1, 'trade_action': 'sell'})
