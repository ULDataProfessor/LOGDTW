...
class QuestSystem:
    ...
    def generate_dynamic_quest(self, player: Player) -> Quest:
        ...
        if mission.rewards.items:
            rewards["items"] = [
                {"name": item_name, "description": item_name, "value": 0, "type": "special"}
    ...
    def accept_quest(self, player: Player, quest_id: str) -> Dict:
        ...
        return {"success": True, "message": f"Quest accepted: {quest.name}", "quest": quest}
    ...
    def update_surface_mission_with_events(
        self, mission: Dict[str, Any], surface, events: List[Dict]
    ) -> Dict[str, Any]:
