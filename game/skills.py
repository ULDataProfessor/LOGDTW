...
class Skill:
    ...
    def gain_experience(self, amount: int) -> Dict:
        ...
        if self.experience >= self.experience_to_next and self.level < self.max_level:
            ...
            if new_unlocks:
                result["messages"].append(f"New abilities unlocked: {', '.join([u.name for u in new_unlocks])}")
    ...
    def _check_new_unlocks(self) -> List[SkillUnlock]:
        ...
        for unlock in unlocks:
            if (unlock.level_required <= self.level and 
                unlock.name not in [u.name for u in self.unlocked_abilities]):
...
class SkillTree:
    ...
    def calculate_skill_bonus(self, skill_name: str, base_value: float, 
                            player_skills: Dict[str, int]) -> float:
    ...
    def get_skill_recommendations(self, player_skills: Dict[str, int], 
                                player_actions: Dict[str, int]) -> List[str]:
        ...
        recommendations = [skill for skill in recommendations 
                         if player_skills.get(skill, 1) < 50]
...
SKILL_UNLOCKS = {
    "Combat": [
        SkillUnlock("Weapon Overcharge", "Increase weapon damage by 50% for 3 rounds", 10, "active", 
                   {"type": "combat_boost", "duration": 3, "effect": "damage_boost"}),
        SkillUnlock("Battle Frenzy", "Attack twice per round when below 25% health", 25, "passive",
                   {"type": "conditional", "trigger": "low_health", "effect": "double_attack"}),
        SkillUnlock("Combat Veteran", "Immunity to fear and morale effects", 50, "passive",
                   {"type": "immunity", "effect": "fear_immunity"}),
        SkillUnlock("Legendary Warrior", "Critical hits have 25% chance to instantly defeat weak enemies", 75, "passive",
                   {"type": "critical", "effect": "instant_kill"})
    ],
    
    "Piloting": [
        SkillUnlock("Evasive Maneuvers", "Double evasion chance for next 3 turns", 15, "active",
                   {"type": "evasion_boost", "duration": 3, "multiplier": 2.0}),
        SkillUnlock("Precision Strike", "Next attack has 100% accuracy and +25% damage", 30, "active",
                   {"type": "attack_boost", "accuracy": 1.0, "damage": 1.25}),
        SkillUnlock("Ace Pilot", "Can perform special maneuvers unavailable to others", 60, "passive",
                   {"type": "unlock", "effect": "special_maneuvers"}),
        SkillUnlock("Legendary Pilot", "Ship automatically evades first attack each combat", 90, "passive",
                   {"type": "auto_defense", "effect": "first_attack_immunity"})
    ],
    
    "Trading": [
        SkillUnlock("Market Insight", "See exact supply/demand for all goods", 20, "passive",
                   {"type": "information", "effect": "market_transparency"}),
        SkillUnlock("Bulk Discount", "Buying 10+ units gives 15% discount", 35, "passive",
                   {"type": "trade_bonus", "threshold": 10, "discount": 0.15}),
        SkillUnlock("Trade Empire", "Establish trade routes that generate passive income", 70, "active",
                   {"type": "passive_income", "effect": "trade_routes"}),
        SkillUnlock("Economic Mastermind", "Can manipulate sector economies", 95, "active",
                   {"type": "market_control", "effect": "price_manipulation"})
    ],
    
    "Engineering": [
        SkillUnlock("Emergency Repair", "Instantly restore 50% hull and shields", 12, "active",
                   {"type": "healing", "hull": 0.5, "shields": 0.5}),
        SkillUnlock("Jury Rig", "Temporarily boost any ship system by 100%", 28, "active",
                   {"type": "system_boost", "multiplier": 2.0, "duration": 5}),
        SkillUnlock("Master Engineer", "All ship systems operate at 150% efficiency", 55, "passive",
                   {"type": "efficiency", "multiplier": 1.5}),
        SkillUnlock("Technological Singularity", "Create unique, one-of-a-kind equipment", 85, "active",
...
class SkillChallenge:
    ...
    def __init__(self, name: str, description: str, required_skills: Dict[str, int],
                 success_rewards: Dict, failure_consequences: Dict):
        self.name = name
        self.description = description
