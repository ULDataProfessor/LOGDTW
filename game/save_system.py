...
class SaveGameSystem:
        ...
        if self.metadata_file.exists():
            try:
                with open(self.metadata_file, "r") as f:
                    data = json.load(f)
                    return {
                        save_id: SaveMetadata(**save_data) for save_id, save_data in data.items()
                    }
    ...
    def _save_metadata(self):
        ...
        try:
            data = {save_id: asdict(metadata) for save_id, metadata in self.save_metadata.items()}
    ...
    def _cleanup_old_backups(self, save_id: str):
        ...
        backups = [
            (backup_id, metadata)
            for backup_id, metadata in self.save_metadata.items()
    ...
    def verify_save_integrity(self, save_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                return {"valid": False, "error": f"Cannot deserialize save data: {e}"}
