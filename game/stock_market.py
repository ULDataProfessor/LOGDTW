...
class StockMarket:
    ...
    def sell_stock(self, player: Player, symbol: str, shares: int) -> Dict:
        ...
        if symbol not in self.player_portfolio or self.player_portfolio[symbol] < shares:
            return {
                "success": False,
                "message": f"Not enough shares. Have {self.player_portfolio.get(symbol, 0)}, need {shares}",
            }
    ...
    def get_portfolio_summary(self) -> Dict:
        ...
        summary = {"total_shares": 0, "total_value": 0, "holdings": []}
...
class BankingSystem:
    ...
    def create_account(self, player: Player, account_type: str, location: str) -> Dict:
        ...
        if account_type not in branch["services"]:
            return {
                "success": False,
                "message": f"{account_type} accounts not available at this branch",
            }
    ...
    def deposit(self, player: Player, account_number: str, amount: float) -> Dict:
        ...
        if player.credits < amount:
            return {
                "success": False,
                "message": f"Not enough credits. Have {player.credits}, need {amount}",
            }
    ...
    def withdraw(self, player: Player, account_number: str, amount: float) -> Dict:
        ...
        if account.balance < amount:
            return {
                "success": False,
                "message": f"Insufficient funds. Balance: {account.balance}, requested: {amount}",
            }
