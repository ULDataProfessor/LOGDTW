...
class World:
        ...
        for sid in range(1, 11):
            if not self.sector_repo.get_sector(sid):
                rec = {
                    "id": sid,
                    "name": (
                        self.sector_names.get(sid, f"Sector {sid}")
                        if hasattr(self, "sector_names")
                        else f"Sector {sid}"
                    ),
                    "faction": "Federation",
                    "region": "Federation",
    ...
    def get_or_create_sector(self, sector_id: int) -> Dict:
        ...
        candidates = [
            i
            for i in range(max(1, sector_id - 7), min(self.max_sectors, sector_id + 7))
            if i != sector_id
        ]
    ...
    def _get_connection_danger(self, connection_type: str) -> int:
        ...
        danger_levels = {"federation": 1, "neutral": 3, "enemy": 8, "hop": 2, "skip": 6, "warp": 4}
    ...
    def get_available_jumps(self) -> List[Dict]:
        ...
        for connection in self.sector_connections[self.current_sector]:
            ...
            sector_locations = [
                ...
                if loc.sector == connection.destination_sector
            ]
            if sector_locations:
                available_jumps.append(
                    {
                        ...
                        "faction": self.sector_factions.get(
                            connection.destination_sector, "Unknown"
                        ),
                    }
                )
    ...
    def jump_to_sector(self, sector_number: int, player) -> Dict:
        ...
        if player.fuel < connection.fuel_cost:
            return {
                "success": False,
                "message": f"Not enough fuel. Need {connection.fuel_cost}, have {player.fuel}",
            }
    ...
    def trigger_event(self, player) -> Optional[Dict]:
        ...
        result = {"name": event["name"], "description": event["description"], "outcome": outcome}
    ...
    def instant_jump(self, destination: str) -> bool:
        ...
        # Discover the sector
        self.discovered_sectors.add(
            self.sector_names.get(dest_location.sector, str(dest_location.sector))
        )
    ...
    def get_map_display(self) -> str:
        ...
        for conn in self.map_data["connections"]:
            ...
            if loc1 in self.map_data["locations"] and loc2 in self.map_data["locations"]:
                x1, y1 = (
                    self.map_data["locations"][loc1]["x"],
                    self.map_data["locations"][loc1]["y"],
                )
                x2, y2 = (
                    self.map_data["locations"][loc2]["x"],
                    self.map_data["locations"][loc2]["y"],
                )
        ...
        if current_loc and current_loc.name in self.map_data["locations"]:
            x, y = (
                self.map_data["locations"][current_loc.name]["x"],
                self.map_data["locations"][current_loc.name]["y"],
            )
            if 0 <= x < map_width and 0 <= y < map_height:
                map_grid[y][
                    x
                ] = f"[bold red]{self.map_data['locations'][current_loc.name]['symbol']}[/bold red]"
    ...
    def land_on_planet(self):
        ...
        if location and location.location_type == "planet":
            ...
            return {
                "success": True,
                "message": f"You have landed on {location.name}. Use n/s/e/w to move.",
            }
    ...
    def collect_surface_resource(self, player):
        if self.on_planet_surface and self.planet_surface:
            ...
            if resource:
                item = Item(
                    name=resource.title(),
                    description=f"{resource} gathered from surface",
                    value=10,
                    item_type="resource",
                )
                if player.add_item(item):
