...
class EnhancedCombatSystem:
    ...
    def start_enhanced_combat(
        self, player_ship: CombatShip, enemy_ship: CombatShip, environment: Dict = None
    ) -> bool:
    ...
    def _apply_environmental_effects(self):
        ...
        if env.get("solar_flare"):
            self.combat_log.append(
                "☀️ Solar flare activity - energy weapons boosted, shields weakened"
            )
    ...
    def execute_combat_action(
        self, actor: str, action: CombatAction, target: str = None, weapon_index: int = 0
    ) -> Dict:
    ...
    def _execute_attack(self, attacker: CombatShip, target: CombatShip, weapon_index: int) -> Dict:
        ...
        if random.random() <= hit_chance:
            ...
            result["message"] = (
                f"{attacker.name} hits {target.name} with {weapon.name} for {damage} damage!"
            )
    ...
    def _execute_defend(self, defender: CombatShip) -> Dict:
        ...
        if defender.shields < defender.max_shields:
            ...
            message = (
                f"{defender.name} raises defenses and regenerates {shield_regen} shield power!"
            )
        else:
    ...
    def create_combat_ship_from_player(self, player) -> CombatShip:
        ...
        defenses = [
            Defense(name="Energy Shields", type=DefenseType.SHIELDS, protection=20, energy_cost=5),
            Defense(name="Hull Armor", type=DefenseType.ARMOR, protection=15, energy_cost=0),
        ]
