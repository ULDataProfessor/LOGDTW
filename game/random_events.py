...
class RandomEventSystem:
    ...
    def check_for_events(
        self, context: EventContext, game_state: Dict[str, Any]
    ...
    def handle_event(
        self, event: RandomEvent, game_state: Dict[str, Any], choice: str = None
    ) -> EventOutcome:
        ...
        if event.event_type == EventType.MALFUNCTION:
            ...
            return EventOutcome(
                success=False, message=f"Unknown event type: {event.event_type}", effects={}
            )

    def _handle_malfunction(
        self, event: RandomEvent, game_state: Dict[str, Any], choice: str = None
    ) -> EventOutcome:
        ...
        if choice is None:
            return EventOutcome(
                ...
                effects={"health_damage": damage, "fuel_loss": fuel_loss},
    ...
    def _handle_storm(
        ...
        return EventOutcome(
            ...
            effects={"health_damage": damage, "sensor_impairment": sensor_impairment_duration},
        )
    ...
    def _handle_friendly_trader(
        ...
        if choice is None:
            return EventOutcome(
                ...
                effects={"trade_offer": True, "offered_good": offered_good, "discount": discount},
        ...
        if choice == "accept":
            return EventOutcome(
                ...
                effects={"special_trade": True, "good": offered_good, "discount": discount},
            )
        elif choice == "decline":
            return EventOutcome(
                success=True, message="You politely decline the trader's offer.", effects={}
            )

        return EventOutcome(success=False, message="Invalid choice for trader encounter.")

    def _handle_mysterious_signal(
        self, event: RandomEvent, game_state: Dict[str, Any], choice: str = None
    ) -> EventOutcome:
        ...
        if choice == "investigate":
            outcomes = [
                (
                    "alien_technology",
                    "You discover alien technology!",
                    {"credits": 2000, "experience": 200},
                ),
            ...
            return EventOutcome(success=outcome_type != "trap", message=message, effects=effects)
        elif choice == "ignore":
            return EventOutcome(
                success=True,
    ...
    def save_to_file(self, save_path: Path):
        ...
        for event_id, event in self.events.items():
            data["events"][event_id] = {"last_triggered": event.last_triggered}
