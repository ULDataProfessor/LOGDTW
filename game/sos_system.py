...
class SOSSystem:
    ...
    def generate_distress_signal(
        self, player_coordinates: Tuple[int, int, int]
        ...
        description = self._generate_distress_description(
            ship_name, ship_type, distress_type, severity
        )
    ...
    def _generate_distress_description(
        self, ship_name: str, ship_type: str, distress_type: str, severity: int
    ) -> str:
        ...
        base_description = random.choice(
            descriptions.get(distress_type, ["Unknown emergency situation."])
        )
    ...
    def attempt_rescue(self, player: Player, signal_id: str) -> Dict:
        ...
        if player.fuel < required_fuel:
            return {
                "success": False,
                "message": f"Not enough fuel. Need {required_fuel}, have {player.fuel}",
            }

        if player.energy < required_energy:
            return {
                "success": False,
                "message": f"Not enough energy. Need {required_energy}, have {player.energy}",
            }
    ...
    def get_rescue_statistics(self) -> Dict:
        ...
        if not self.completed_rescues:
            return {"total_rescues": 0, "total_rewards": 0, "average_reward": 0}
    ...
    def get_nearby_signals(
        self, player_coordinates: Tuple[int, int, int], max_distance: int = 50
    ) -> List[Tuple[str, DistressSignal, float]]:
